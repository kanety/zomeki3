class BizCalendar::Public::Node::PlacesController < BizCalendar::Public::Node::BaseController
  ...
  def index
    ...
    @places.each do |place|
      ...
      @holidays[place.id]           = BizCalendar::BussinessHoliday.with_state(:public).all_with_place_and_criteria(place, criteria).order(:holiday_start_date)
      @exception_holidays[place.id] = BizCalendar::ExceptionHoliday.with_state(:public).all_with_place_and_criteria(place, criteria).order(:start_date)

      criteria[:repeat_type] = 'not_null'
      @repeat_holidays[place.id] = BizCalendar::BussinessHoliday.with_state(:public).all_with_place_and_criteria(place, criteria).order(:holiday_start_date)
    end
  end
  ...
  def bussiness_times
    ...
    if @piece.target_today?
      ...
      @places.each do |place|
        ...
        @exception_holidays[place.id] = BizCalendar::ExceptionHoliday.with_state(:public).all_with_place_and_criteria(place, criteria).order(:start_date)
        holidays = BizCalendar::BussinessHoliday.with_state(:public).all_with_place_and_criteria(place, criteria).order(:holiday_start_date)

        @holidays[place.id] = Hash.new()
        holidays.each{ |h| @holidays[place.id][h.type.id] = [h.type.name, h.type.title] }

        criteria[:repeat_type] = 'not_null'
        repeat_holidays = BizCalendar::BussinessHoliday.with_state(:public).all_with_place_and_criteria(place, criteria)
