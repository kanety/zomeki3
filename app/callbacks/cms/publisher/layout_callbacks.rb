class Cms::Publisher::LayoutCallbacks < PublisherCallbacks
  ...
  def enqueue_nodes
    nodes = Cms::Node.with_state(:public).where(layout_id: @layouts.map(&:id))
    Cms::Publisher.register(@site.id, nodes)
  end

  def enqueue_categories
    cats = GpCategory::CategoryType.with_state(:public).where(layout_id: @layouts.map(&:id))
                                   .flat_map { |cat_type| cat_type.public_categories }
    cats += GpCategory::Category.with_state(:public).where(layout_id: @layouts.map(&:id))
    Cms::Publisher.register(@site.id, cats.uniq)
  end

  def enqueue_organization_groups
    ogs = Organization::Group.with_state(:public).with_layout(@layouts.map(&:id))
  ...
  def enqueue_gnav_menu_items
    ...
    nodes = Cms::Node.with_state(:public).where(content_id: contents)
    Cms::Publisher.register(@site.id, nodes)
  end

  def enqueue_docs
    docs = GpArticle::Doc.with_state(:public).where(layout_id: @layouts.map(&:id)).select(:id)
